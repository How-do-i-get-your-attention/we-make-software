...
<h2>"Your memory also gets old."</h2>
  by:<br/>  
Junior AKA Good virus, Inventor and Founder<br/>
"I'm not here to unlock the secrets of programming languages or debate the supremacy of platforms. My allegiance lies with Windows, and I craft my code in the sanctum of Visual Studio – that's where I thrive. But in the vast expanse of the digital cosmos, nothing looms larger than the omnipotent global network. Let's embark on a cosmic quest to establish contact with this colossal entity.
<br/>
In my interstellar journey through the coding universe, I wield an arsenal of programming languages, each chosen for its affinity with the system at hand. For the Windows domain, I harness the power of C++, a language as ancient and mysterious as the dark matter that permeates the universe. Across the celestial plane of Mac, iOS, and Apple products, I navigate with the nimble elegance of Swift, a language as swift as its name implies. And in the enigmatic realm of Android, I dance with the cosmic forces of Kotlin, a language as vibrant and unpredictable as the cosmic rays that traverse the cosmos.
<br/>
But fear not, fellow voyager, for I am armed with the most potent tools of the trade: Android Studio for forging connections with the Android frontier, Visual Studio for charting a course through the vast expanses of Windows, and Xcode for unraveling the mysteries of Apple's cosmic creations.
<br/>
And remember, amidst the cosmic symphony of coding, I'm not here to venture into the depths of kernel programming. That's a realm best left to the brave souls who dare to traverse the outer edges of the digital universe, where only the boldest of coders tread."<br/><br/>


"I need your help if you want to help me. hello@how-do-i-get-your-attention.com"

I placed the project in a folder named we-make-software on the C drive. What are you going to do?
I've attempted to consolidate everything within Windows. I did try to work with Linux, but I was never certain about the origins of various components within the Linux platform, making me feel that Linux is uncertain.

# <img src="service.png" width="24"/> Service

The executable is located in `C:\we-make-software\x64\Release`. Whenever I create a new project, I always remove the Debug configuration because I don't see the need for it when running the application from Visual Studio. 
Additionally, I add a post-build event: `del "$(TargetDir)$(TargetName).pdb"`, because I don't require the PDB files. As a developer, I understand which files are necessary for my work.
I always use header files to upgrade a level, which means I don’t have to repeat myself. 
I utilize these headers to establish communication with my other DLLs or to create a structure.
The service can run by visual studio or by a server its made like if administrator just open it whitout visual studio it will run as service.
When you or I test it in Visual Studio, we get a simulation of how the service will operate. Essentially, there's a 'start' event that is triggered when the service begins, and a 'stop' event that executes when the service needs to be stopped, which could be due to a server restart.
The header in the service includes the macros `TRY_CATCH_ERROR` and `END_TRY_CATCH`. If you or i use these, it can print out an error by using `throw std::runtime_error("error message.");`
If it's run by the service, it operates under the System account, a local account that does not require administrator privileges. The point is, this application or service can run with Administrator rights, allowing us to test it—not debug, but test it.

The output will be `Service.exe`.

Its basily me who loading your script or my script this is how i do.

I need to know the thread thats run and how long time it take to response.